...
Beaucoup d'endroits où on creer le nom du scheme avec suff : 
     ...
     - indschemes 252 : lorsqu'un inductif est def par l'utilisateur
       just avant l'appel à ind_tables.define_individual_scheme
       ici on creer le tableau elims: (Ind_tables.individual Ind_tables.scheme_kind * string option) list =
...
** TODO Aller voir dans elpi comment c est fait
...
** TODO type handle side_effect : aller voir
     Gère des dependance avec d'autres schemas, d autres noms globaux comme les schemas.

** TODO Dans elimschemes : build_induction_scheme_in_type c est le f de la fonction precedante.
...
** TODO Generaliser la syntaxe de scheme equality pour qu on puisse faire la meme chose que avec induction : avoir un ident
...
** TODO Quelle syntaxe on va donner pour les schemas elpi
