...
pr_opt : verifier celui qui met un espace ou rien
pr_opt_spc :  
pour ne pas faire le test du none some
...
** centraliser plus de choses dans ind_tables : 
...
-> dans la table declare_scheme_object : que la clef soit une liste de string comme Equality ou boolean equality.

define_individual_scheme_base : kind devient une liste de string


** indschemes :  
do_scheme va bcp changer, il restera plus que do mutual scheme
et on va supprimer so scheme equality
